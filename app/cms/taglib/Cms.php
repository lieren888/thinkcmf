...
class Cms extends TagLib
{
    ...
    public function tagArticles($tag, $content)
    {
        ...
        $order         = empty($tag['order']) ? 'a.published_time DESC' : $tag['order'];
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\cms\service\ApiService::articles([
    'field'   => {$field},
    'where'   => {$where},
    'limit'   => {$limit},
    'order'   => {$order},
...
parse;
    ...
    public function tagTagArticles($tag, $content)
    {
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\cms\service\ApiService::tagArticles([
    'field'   => {$field},
    'where'   => {$where},
    'limit'   => {$limit},
    'order'   => {$order},
...
<?php
if(!empty({$cid})){
    \$__BREADCRUMB_ITEMS__ = \app\cms\service\ApiService::breadcrumb({$cid},{$self});
?>
...
parse;
    ...
    public function tagCategories($tag, $content)
    {
        ...
        $returnVarName = 'cms_categories_data';
        if (strpos($ids, '$') === 0) {
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\cms\service\ApiService::categories([
    'where'   => {$where},
    'order'   => '{$order}',
    'ids'     => {$ids}
]);
...
parse;
    ...
    {
        $item          = empty($tag['item']) ? 'vo' : $tag['item'];//循环变量名
        $returnVarName = 'cms_sub_categories_data';
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\cms\service\ApiService::subCategories({$categoryId});
 ?>
<volist name="{$returnVarName}" id="{$item}">
{$content}
</volist>
parse;
    ...
    public function tagAllSubCategories($tag, $content)
    {
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\cms\service\ApiService::allSubCategories({$categoryId});
 ?>
<volist name="{$returnVarName}" id="{$item}">
{$content}
</volist>
parse;
    ...
    public function tagPage($tag, $content)
    {
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\cms\service\ApiService::page([
    'id'     => {$id}
]);
 ?>
{$content}
